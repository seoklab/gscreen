...
def _plot_boxes(
    ...
    """Publication-ready box plot with significance brackets."""
    data = agg[agg["metric"] == metric]
    palette = sns.color_palette("colorblind", len(ALL_METHODS))
    ...
    method_to_x = {m: i for i, m in enumerate(ALL_METHODS)}
    ...
    ax.set_ylim(top=y_cur + y_step * 0.5)
    fig.tight_layout()
    fig.savefig(output, bbox_inches="tight", dpi=500)
    plt.close(fig)
...
def main(
    ...
    metric_names = ["aucroc", *(f"ef {r * 100:1g}%" for r in ratios)]
    metric_lims = [(0.0, 1.0), *[(0.0, None) for _ in ratios]]
    ...
    for metric, lims in zip(metric_names, metric_lims):
        safe_name = metric.replace(" ", "_").replace("%", "pct")
        out_path = results / f"crossval_{safe_name}.svg"
        _plot_boxes(agg, metric, wilcoxon_df, out_path, lims)
